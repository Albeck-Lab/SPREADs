...
% FIX: BAD FRAME tp 7 in XY..... 1,2,3 RUN SEPREATELY

function [ip, op] = celltracer_v2_2023_01_10_pHBE_EKAREN4_ALI_Cyto_Plate_1
...
ip.indsz.t  = [252];   %Number of Time points
...
[ip.bkg(37).reg]    = deal([50, 50; 1550, 1550]);     %Region (or fixed values)
...
ip.xyshift.frame = [6,12];      %Time indices of first shifted frames
...
ip.xyshift.badtime   = [7];  %Time points bad for tracking (optional)
...
%        *+ = Ensure for Field Flattening )
    ...
    ip.bkmd.exp.ExVolt     =   {10,20};     %Relative voltage (0-100) ++
...
op.xypos    = [1:3,37];       %Indices of XY positions to process
op.zstack   = [1];
op.trng     = [1:252];       %Start and End indices of Time to process
