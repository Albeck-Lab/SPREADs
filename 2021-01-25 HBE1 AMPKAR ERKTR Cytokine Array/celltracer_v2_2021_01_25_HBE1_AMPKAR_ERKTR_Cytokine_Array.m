...
function [ip, op] = celltracer_v2_2021_01_25_HBE1_AMPKAR_ERKTR_Cytokine_Array()
...
[ip.bkg(1).reg]    = deal([50, 50; 1230, 1030]);  
...
[ip.bkg([2:49]).altxy]  = deal([1]);     	%Index of alternate XY to use for bkg
[ip.bkg(1).bkonly] = deal(true);  	%Is this XY only for background
...
ip.xyshift.frame = [23];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {100, 150, 250, 200};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {7, 15, 25, 30};     %Relative voltage (0-100) ++
...
op.seg.minD = 12;      	%Minimum nuclear diameter (um)
...
op.seg.Extent = [0.65, 0.92]; %Minimum fraction of bounding box filled 
%   Extent range is [0-1], value for a cirlce is PI/4
op.seg.maxSmooth = 0.1;     %Maximum fraction of convex hull without mask

op.seg.sigthresh = [200,600]; %[Optional] Minimum intensity of 'good' signal
...
op.msk.cgap = 1;    %[Optional] Approx. # pixels to expand nuc-cyt mask gap
