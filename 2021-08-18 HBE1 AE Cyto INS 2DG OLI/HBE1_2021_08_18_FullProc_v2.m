...
%% Live cell Analysis
sensors = {'ERKTR_RFP','RFP_Nuc','AMPKAR','Area_YFP_Cyt'}; % added for ampk 'AMPKAR','Area_YFP_Cyt'
fts(1).t = 'Min'; fts(1).c = 'ERKTR'; fts(1).p = 0.02;
fts(2).t = 'Max'; fts(2).c = 'ERKTR'; fts(2).p = 4; 
fts(3).t = 'minmax'; fts(3).c = 'RFP_Nuc'; fts(3).p = 400;
fts(4).t = 'Min'; fts(4).c = 'AMPKAR'; fts(4).p = 0.5; % added for AMPK data
fts(5).t = 'Max'; fts(5).c = 'AMPKAR'; fts(5).p = 0.7; % added for AMPK data
fts(6).t = 'Min'; fts(6).c = 'Area_YFP_Cyt'; fts(6).p = 20; % added for AMPK data

filterp.dlength = 120; % 12 hr min for tracks (was 60)
filterp.fts = fts;
dataloc = DatalocHandler('sensors',sensors,'filterp',filterp,'saveit',false,'load',false);
plot_by_ND('treatment', dataloc, 'plottype', {'mean','sorted stacks'},'channel', 'AMPKAR','tx_order', 1,'aftertreatment',1,'tmaxaftertx',24,'combinexys',true,'zerohrtx',2)
plot_by_ND('treatment', dataloc, 'plottype', {'mean','sorted stacks'},'channel', 'ERKTR','tx_order', 1,'aftertreatment',1,'tmaxaftertx',24,'combinexys',true,'ymn',0.5,'ymx',2.5)
