...
function [ip, op] = celltracer_v2_2021_08_18_HBE1_AE_Cyto_INS_2DG_OLI_v2()
...
ip.indsz.xy = [77];   %Number of XY positions
ip.indsz.z  = [1];   %Number of Z positions
ip.indsz.t  = [264];   %Number of Time points
...
[ip.bkg(61).reg]    = deal([820, 1008; 860, 1075]); % was not here
[ip.bkg(76).reg]    = deal([820, 1008; 860, 1075]); % was 50,50; 550,550
...
%[ip.bkg([1:75,77]).altxy]  = deal(76);     	%Index of alternate XY to use for bkg
[ip.bkg([1:30,69:75,77]).altxy]  = deal(76);     	%Index of alternate XY to use for bkg - NO INS
[ip.bkg([31:60,62:68]).altxy]  = deal(61);     	%Index of alternate XY to use for bkg - PLUS INS


[ip.bkg(76).bkonly] = deal(true);  	%Is this XY only for background
[ip.bkg(61).bkonly] = deal(false);  	%Is this XY only for background
...
ip.xyshift.frame = [20,41,242];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {25, 50, 150, 175};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {14, 30, 50, 50};     %Relative voltage (0-100) ++
...
%op.xypos    = [1:77];       %Indices of XY positions to process
op.xypos    = [61:77];       %Indices of XY positions to process
op.trng     = [1:264];       %Start and End indices of Time to process
op.nW       = 12;        %Number of parallel workers to use
%   Optional procedures
...
op.seg.chan = [1, 3];       %Color channel on which to segment  !!
...
op.seg.minD = 10;      	%Minimum nuclear diameter (um)
...
op.seg.sigthresh = [200,250]; %[Optional] Minimum intensity of 'good' signal
op.seg.hardsnr = false;  %Typically kept to FALSE.  TRUE makes the signal 
%   threshold 'hard', enforcing cutoff of any pixels below it.
op.seg.nrode = 2;  %[Optional] Approx. number of pixels to erode nuc mask
...
op.msk.aggfun(4).chan = {'CFP'};
op.msk.aggfun(4).loc = {'Cyt'};
op.msk.aggfun(4).fun = @median;

op.msk.aggfun(5).name = 'Med';
op.msk.aggfun(5).chan = {'YFP'};
op.msk.aggfun(5).loc = {'Cyt'};
op.msk.aggfun(5).fun = @median;
...
op.msk.nfilt = true; %[Optional] TRUE to filter cyto mask by nuc channel
...
% set(figgies(4),'Position', [0, 0, 1280, 1080]); saveas(figgies(4), [dataloc.fold.fig, '\', 'segcheck_nuc_xy20_t20_nrode', num2str(op.msk.nrode), '_cgap', num2str(op.msk.cgap), '_cwidth', num2str(op.msk.cwidth), '.png'])
% set(figgies(3),'Position', [0, 0, 1280, 1080]); saveas(figgies(3), [dataloc.fold.fig, '\', 'segcheck_cyt_xy20_t20_nrode', num2str(op.msk.nrode), '_cgap', num2str(op.msk.cgap), '_cwidth', num2str(op.msk.cwidth), '.png'])
