...
function [ip, op] = celltracer_v2_2023_01_10_pHBE_EKAREN4_ALI_Cyto_Plate_2
...
ip.indsz.xy = [37];   %Number of XY positions
...
[ip.bkg(37).reg]    = deal([100, 100; 550, 550]);     %Region (or fixed values)
[ip.bkg(:).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(:).dyn]    = deal(true);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:36]).altxy]  = deal([37]);     	%Index of alternate XY to use for bkg
ip.bkg([37]).bkonly = true;  	%Is this XY only for background
...
ip.xyshift.frame = [8,13,183,218];      %Time indices of first shifted frames
...
%        *+ = Ensure for Field Flattening )
    ...
    ip.bkmd.cam.PixNumX    =   2560;       %Number of Pixels in frame, X **
    ip.bkmd.cam.PixNumY    =   2160;       %Number of Pixels in frame, Y **
    ip.bkmd.cam.BinSizeX   =   2;          %Number of Pixels per bin, X
    ip.bkmd.cam.BinSizeY   =   2;          %Number of Pixels per bin, Y
    ip.bkmd.cam.CamSizeX   =   1280;       %Camera sensor X resolution *+
    ip.bkmd.cam.CamSizeY   =   1080;       %Camera sensor Y resolution *+
    ...
    ip.bkmd.exp.Exposure   =   {200,250};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {15,80};     %Relative voltage (0-100) ++
...
op.xypos    = [1:37];       %Indices of XY positions to process
op.zstack   = [1];
...
op.seg.maxSmooth = 0.3;     %Maximum fraction of convex hull without mask 
op.seg.sigthresh = [300]; %[Optional] Minimum intensity of 'good' signal
