...
function [ip, op] = celltracer_v2_2021_07_28_HBE1_AMPKAR_ERKTR_Cytokine_INS_OLI_2DG()
...
ip.indsz.xy = [89];   %Number of XY positions
ip.indsz.z  = [1];   %Number of Z positions
ip.indsz.t  = [230];   %Number of Time points
...
[ip.bkg(89).reg]    = deal([100, 50; 600, 550]);
...
[ip.bkg([1:88]).altxy]  = deal(89);     	%Index of alternate XY to use for bkg
[ip.bkg(89).bkonly] = deal(true);  	%Is this XY only for background
...
ip.xyshift.frame = [18,38];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {50, 75, 150, 150};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {12, 20, 40, 40};     %Relative voltage (0-100) ++
...
op.xypos    = [1:89];       %Indices of XY positions to process
op.trng     = [1:230];       %Start and End indices of Time to process
...
op.seg.sigthresh = [300,350]; %[Optional] Minimum intensity of 'good' signal
