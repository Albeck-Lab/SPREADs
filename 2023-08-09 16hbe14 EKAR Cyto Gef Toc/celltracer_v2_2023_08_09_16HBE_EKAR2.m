...
function [ip, op] = celltracer_v2_2023_08_09_16HBE_EKAR2()
addpath('\\albecklab.mcb.ucdavis.edu\Data\Code\Nick','\\albecklab.mcb.ucdavis.edu\Data\Code\Cell Trace','\\albecklab.mcb.ucdavis.edu\Data\Code\Image Analysis','\\albecklab.mcb.ucdavis.edu\data\bfmatlab\')
...
ip.indsz.xy = [91];   %Number of XY positions
ip.indsz.z  = [1];   %Number of Z positions
ip.indsz.t  = [261];   %Number of Time points
...
[ip.bkg(91).reg]    = deal([50, 50; 550, 550]); %Region (or fixed values)
[ip.bkg(91).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(91).dyn]    = deal(true);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:90]).altxy]  = deal(91);     	%Index of alternate XY to use for bkg
[ip.bkg(91).bkonly] = deal(true);  	%Is this XY only for background
...
ip.xyshift.frame = [3,4,8,9];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {150, 100};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {20, 25};     %Relative voltage (0-100) ++
...
op.xypos    = [65:91];       %Indices of XY positions to process
op.trng     = [1:261];       %Start and End indices of Time to process
...
% Nick Settings
op.seg.maxD = 20;       %Maximum nuclear diameter (um)
...
op.seg.sigthresh = [1000]; %[Optional] Minimum intensity of 'good' signal
...
% 
pst = [];
pst = iman_segcheck(ip, op, 't',20, 'xy',34, 'pastinfo',pst);
pst = iman_segcheck(ip, op, 't',21, 'xy',34, 'pastinfo',pst);
pst = iman_segcheck(ip, op, 't',22, 'xy',34, 'pastinfo',pst);
% 
figgies = findobj('Type', 'figure');
set(figgies(1),'Position', [0, 0, 1280, 1080]); saveas(figgies(1), [dataloc.fold.fig, '\', 'segcheck_nuc_xy34_t20.png'])
set(figgies(2),'Position', [0, 0, 1280, 1080]); saveas(figgies(2), [dataloc.fold.fig, '\', 'segcheck_nuc_xy34_t21.png'])
set(figgies(3),'Position', [0, 0, 1280, 1080]); saveas(figgies(3), [dataloc.fold.fig, '\', 'segcheck_nuc_xy34_t22.png'])
close(figgies(1)); close(figgies(2)); close(figgies(3))
...
iman_statusreport({'nick'},'happy celltracer');
