...
function [ip, op] = celltracer_v2_2022_10_18_pHBE_EKAREN4_ALI_Cytokines_2
...
ip.fname = 'D:\Holder\2022-10-18 pHBE EKAREN4 ALI Cytokines.nd2';
%   Target save name for processed data files (include path)
ip.sname = '\\mcb.ucdavis.edu\shared\Data\mcb_labs\albeck\ProcessedData\Nick\2022-10-18 pHBE EKAREN4 ALI Cytokine\2022-10-18 pHBE EKAREN4 ALI Cytokine';  
...
ip.indsz.xy = [41,];   %Number of XY positions
ip.indsz.z  = [1];   %Number of Z positions
ip.indsz.t  = [247];   %Number of Time points
...
[ip.bkg(21).reg]    = deal([50, 50; 1550, 1550]);     %Region (or fixed values)
[ip.bkg(:).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(:).dyn]    = deal(false);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:20,22:41]).altxy]  = deal([21]);     	%Index of alternate XY to use for bkg
ip.bkg(21).bkonly = true;  	%Is this XY only for background
...
ip.xyshift.frame = [7];      %Time indices of first shifted frames
...
%        *+ = Ensure for Field Flattening )
    ...
    ip.bkmd.exp.Exposure   =   {200,200};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {30,30};     %Relative voltage (0-100) ++
    %   IF using the SPECTRAX light source, also define the following:
    ip.bkmd.exp.ExLine     =   {[4],[2]};   %Excitation line
    ip.bkmd.exp.ExWL       =   {[514],[440]};   %Wavelength
    %   ----- ----------------------------------------------- -----
...
op.xypos    = [21:41];       %Indices of XY positions to process
op.zstack   = [1];
op.trng     = [1:247];       %Start and End indices of Time to process
...
op.seg.sigthresh = [2500]; %[Optional] Minimum intensity of 'good' signal
...
%  pst = [];
%  pst = iman_segcheck(ip, op, 't',7, 'xy',5, 'pastinfo',pst);
%  pst = iman_segcheck(ip, op, 't',8, 'xy',5, 'pastinfo',pst);
%  pst = iman_segcheck(ip, op, 't',9, 'xy',5, 'pastinfo',pst);
% 
% %
