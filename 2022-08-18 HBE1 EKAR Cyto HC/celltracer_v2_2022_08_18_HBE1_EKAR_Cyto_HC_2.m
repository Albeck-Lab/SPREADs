...
function [ip, op] = celltracer_v2_2022_08_18_HBE1_EKAR_Cyto_HC_2()
...
ip.indsz.t  = [509];   %Number of Time points
ip.indsz.c  = [2];   %Number of color Channels
ip.tsamp    = [3];   %Time between samples (MUST BE FILLED)
...
[ip.bkg(49).reg]    = deal([50, 50; 550, 550]); %Region (or fixed values)
[ip.bkg(:).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(:).dyn]    = deal(true);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:48]).altxy]  = deal(49);     	%Index of alternate XY to use for bkg
[ip.bkg(49).bkonly] = deal(true);  	%Is this XY only for background
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.ExVolt     =   {15, 15};     %Relative voltage (0-100) ++
...
op.xypos    = [31,49];       %Indices of XY positions to process
op.trng     = [1:509];       %Start and End indices of Time to process
...
% Nick Settings
op.seg.maxD = 28;       %Maximum nuclear diameter (um)
...
op.trk.gapwin = 15;         % Time window (in minutes) for gap closing
