...
function [ip, op] = celltracer_v2_2023_09_28_HBE1_EKAR()
...
ip.indsz.xy = [73];   %Number of XY positions
ip.indsz.z  = [1];   %Number of Z positions
ip.indsz.t  = [293];   %Number of Time points
...
[ip.bkg(73).reg]    = deal([50, 50; 550, 550]); %Region (or fixed values)
[ip.bkg(73).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(73).dyn]    = deal(true);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:72]).altxy]  = deal(73);     	%Index of alternate XY to use for bkg
[ip.bkg(73).bkonly] = deal(true);  	%Is this XY only for background
...
ip.xyshift.frame = [17,18];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {50, 100};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {10, 17};     %Relative voltage (0-100) ++
...
op.xypos    = [30:35,73];       %Indices of XY positions to process
op.trng     = [1:293];       %Start and End indices of Time to process
...
op.seg.minD = 7;      	%Minimum nuclear diameter (um)
...
op.seg.sigthresh = [600]; %[Optional] Minimum intensity of 'good' signal
