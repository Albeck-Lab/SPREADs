...
function [ip, op] = celltracer_v2_2023_09_06_HBE1_EKAR()
...
ip.indsz.t  = [263];   %Number of Time points
...
[ip.bkg(96).reg]    = deal([50, 50; 550, 550]); %Region (or fixed values)
[ip.bkg(96).fix]   = deal(false);	%Is .reg a fixed intensity value
[ip.bkg(96).dyn]    = deal(true);  	%Is this XY bkg dynamic (sample each frame)?
[ip.bkg([1:95]).altxy]  = deal(96);     	%Index of alternate XY to use for bkg
[ip.bkg(96).bkonly] = deal(true);  	%Is this XY only for background
...
ip.xyshift.frame = [14,15,19,20];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.Exposure   =   {75, 75};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {15, 20};     %Relative voltage (0-100) ++
...
op.xypos    = [1:3,96];       %Indices of XY positions to process
op.trng     = [1:263];       %Start and End indices of Time to process
op.nW       = 24;        %Number of parallel workers to use
%   Optional procedures
...
op.seg.Extent = [0.6, 0.85]; %Minimum fraction of bounding box filled 
%   Extent range is [0-1], value for a cirlce is PI/4
op.seg.maxSmooth = 0.15;     %Maximum fraction of convex hull without mask

op.seg.sigthresh = [400]; %[Optional] Minimum intensity of 'good' signal
...
% % 
pst = [];
pst = iman_segcheck(ip, op, 't',70, 'xy',3, 'pastinfo',pst);
pst = iman_segcheck(ip, op, 't',71, 'xy',3, 'pastinfo',pst);
pst = iman_segcheck(ip, op, 't',72, 'xy',3, 'pastinfo',pst);

% 
figgies = findobj('Type', 'figure');
set(figgies(1),'Position', [0, 0, 1280, 1080]); saveas(figgies(1), [dataloc.fold.fig, '\', 'segcheck_nuc_xy3_t70.png'])
set(figgies(2),'Position', [0, 0, 1280, 1080]); saveas(figgies(2), [dataloc.fold.fig, '\', 'segcheck_nuc_xy3_t71.png'])
set(figgies(3),'Position', [0, 0, 1280, 1080]); saveas(figgies(3), [dataloc.fold.fig, '\', 'segcheck_nuc_xy3_t72.png'])
