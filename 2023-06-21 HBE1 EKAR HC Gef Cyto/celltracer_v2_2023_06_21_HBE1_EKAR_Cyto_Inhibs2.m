...
function [ip, op] = celltracer_v2_2023_06_21_HBE1_EKAR_Cyto_Inhibs2()
...
ip.indsz.t  = [226];   %Number of Time points
...
[ip.bkg(96).reg]    = deal([50, 50; 200, 200]); %Region (or fixed values)
...
ip.xyshift.frame = [4,9,19];      %Time indices of first shifted frames
...
%       (** = Critical for all runs, ++ = Only for objbias)
    ...
    ip.bkmd.exp.FPhore     =   {'mTurq2', 'YPet', 'mCard'};
    ...
    ip.bkmd.exp.Exposure   =   {100, 100, 100};  %Exposure times (ms) ++
    ip.bkmd.exp.ExVolt     =   {15, 10, 25};     %Relative voltage (0-100) ++
    %   IF using the SPECTRAX light source, also define the following:
      ip.bkmd.exp.ExLine     =   {[2],[4],[6]};   %Excitation line
      ip.bkmd.exp.ExWL       =   {[440],[510],[575]};   %Wavelength
    %   ----- ----------------------------------------------- -----
...
op.xypos    = [31:63,96];       %Indices of XY positions to process
op.trng     = [1:226];       %Start and End indices of Time to process
...
try 
   ...
   iman_statusreport({'nick'},'happy celltracer')
    ...
    iman_statusreport({'nick'},'sad celltracer')
    addpath('\\albecklab.mcb.ucdavis.edu\Data\Code\Nick')
end
